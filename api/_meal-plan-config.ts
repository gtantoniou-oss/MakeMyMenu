...
Every ingredient in every recipe and prep task must include BOTH:
1. A practical conventional kitchen measurement in quantity and unit, such
   as cups, tablespoons, teaspoons, fluid ounces, ounces, pounds, pieces,
   cloves, cans, or packages.
2. Its corresponding weight in the numeric grams field.

Use ingredient-specific density when converting volume to grams; do not
assume that every milliliter weighs one gram. For count-based produce, meat,
and packaged items, provide a realistic approximate edible or used weight in
grams. Use decimal grams when needed for small amounts such as salt, yeast,
and spices. Never omit grams, never put units or words inside the grams
field, and never use zero grams for an ingredient that is actually used.
Keep conventional measurements kitchen-friendly rather than replacing them
with metric-only quantities.
...
leftover change, or preference exactly. When feedback contains a
budgetAdjustment, rebuild the menu and consolidated grocery list so the
likely grocery total is at or below targetBudget. Use estimatedCurrentCost,
pricedItems, and pricingStore as practical pricing context. Treat the notes
as protected priorities: for example, "keep the steak night" means preserve
that meal and find savings elsewhere. Do not promise an exact retailer total;
reduce expensive ingredients, reuse ingredients, increase leftovers, and use
lower-cost proteins or produce where permitted.

Rebuild the complete package rather than patching isolated text. Recalculate recipes, servings, ingredient
