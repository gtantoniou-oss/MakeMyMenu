  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        error:
          ...
          "Unable to retrieve the background response."
      });
    }
    ...
    if (!outputText) {
      return res.status(502).json({
        error:
          "The completed response contained no Fridge Cleanup output."
      });
    }

    let fridgeCleanup: unknown;

    try {
      fridgeCleanup = JSON.parse(outputText);
    } catch {
      return res.status(502).json({
        error:
          "The completed response contained invalid JSON."
      });
    }
